...
def notice_device_app(dev: Device, pkg, ver):
    ...
    if len(matching_devs) == 0:
        ...
        Device.update(imei=dev.imei, wifi_mac=dev.wifi_mac, ext_ip=dev.ext_ip, lan_ip=dev.lan_ip, last_noticed=match.last_noticed).where(Device.id == match.id).execute()
        # match.update()
        dev = match
    ...
    if len(existing_devpacks) == 0:
        ...
        for dpk in existing_devpacks:
            dpk.version = ver
            DevicePackage.update(version=ver).where(DevicePackage.id == dpk.id).execute()
...
def get_all_dev_packages():
    ...
    devs = (Device
            .select())
    ipreader = geolite2.reader() if geolite2 is not None else None
    for dev in devs:
        ...
        if len(devpacks) == 0:
            continue
        match = None
        if ipreader is not None and dev.ext_ip:
            try:
